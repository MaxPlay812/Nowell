...
public class Game {
    ...
    public boolean Gamestr() throws InterruptedException {
        ...
        MainTest.TextOutput("                " + "количество верных точек: 6",false);
        Scanner in = new Scanner(System.in);
        num_mines = 6;
        prepare();
        while (!finish) {
            MainTest.TextOutput("                " + "Введите номер строки N и столбца M:",true);
            MainTest.TextOutput("                " + "N=",false);
            int N,M;
            for(;;) {
                N = in.nextInt();
                if (N<=3&N>0){
                    ...
                    MainTest.TextOutput("                " + MainTest.MassErrors[random.nextInt(0, MainTest.MassErrors.length)]+"\t                ",true);
                }
            }
            MainTest.TextOutput("                " + "M=",false);
            for(;;) {
                M = in.nextInt();
                if (M<=3&M>0){
                    break;
            ...
            finish = true;
            if (rez == 1) {
                MainTest.TextOutput("                " + "Вы ПОБЕДИЛИ",true);
                victory=true;
            } else {
                MainTest.TextOutput("                " + "Вы проиграли!",true);
            }
        }
        return victory;
    }
    ...
    public int readCell(int N, int M) throws InterruptedException {
        ...
        for (int i = 1; i < n + 1; ++i) {
            for (int j = 1; j < m + 1; ++j) {
                if ((i == N) && (j == M)) {
                    MainTest.TextOutput("                " + Field[i][j] + " ",false);

                }
                else MainTest.TextOutput("                " + "? ",false);
